...
async def get_status(
    ...
    pnl = await svc.get_today_pnl_summary()
    status["today_realized_pnl"] = pnl["today_realized_pnl"]
    status["today_trades"] = pnl["today_trades"]
    status["cumulative_pnl"] = pnl["cumulative_pnl"]
    status["initial_assets"] = pnl["initial_assets"]
    status["watch_list_detail"] = [
        {"code": c, "name": get_name(c)} for c in status.get("watch_list", [])
    ]

    if status.get("total_cash", 0) <= 0 and pnl["yesterday_total_assets"] > 0:
        status["total_cash"] = pnl["yesterday_total_cash"]
        status["total_assets"] = pnl["yesterday_total_assets"]
...
async def get_event_history(
    ...
    events = await report_repo.get_recent_bot_events(limit=200)
    return [
        {"type": e.type, "message": e.message, "timestamp": e.timestamp, "data": e.data}
        for e in events
    ]
...
async def get_error_history(
    ...
    events = await report_repo.get_recent_bot_events(limit=50)
    return [
        {"type": e.type, "message": e.message, "timestamp": e.timestamp, "data": e.data}
...
async def get_performance(
    ...
    if not already_has_today:
        today_entry = await svc.build_today_performance_entry()
        if today_entry:
...
async def manual_sell(
    ...
    if not body.stock_code:
        raise HTTPException(status_code=400, detail="stock_code가 필요합니다")

    result = await svc.manual_sell(body.stock_code, body.quantity)
    if result.get("error") == "not_found":
        raise HTTPException(status_code=404, detail=result["message"])
    if result.get("error"):
        raise HTTPException(status_code=400, detail=result["message"])
    return {"success": result["success"], "order_no": result["order_no"], "message": result["message"]}
...
async def manual_buy(
    ...
    result = await svc.manual_buy(body.stock_code, body.quantity)
    if result.get("error") == "api_error":
