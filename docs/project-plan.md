...
| [implementation-progress.md](./implementation-progress.md) | 구현 진행 추적. 체크리스트·검증·완료일 기록 | 구현 진행 상황, Phase별 완료 여부 |
...
|------|------|
| 전략 유형 | 규칙 기반 스윙 트레이딩 |
| 매수 조건 | 골든크로스 + 3일 확인 + 거래량 확인(1.5배) + RSI 필터(30~70) + MA(20) 상승 + 품질 필터 + 상승추세 진입(UPTREND_ENTRY) |
...
| 신호 확인 | `SIGNAL_LOOKBACK_DAYS` | `14` | 교차 감지 범위 (일) |
...
| | `RATE_LIMIT_PER_SECOND` | `2` | 초당 API 호출 제한 (모의투자 기준, 실전 시 10) |
