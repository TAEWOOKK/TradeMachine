...
│  │       │  ※ 휴장일이면 SKIP                      │     │
│  │       ▼                                        │     │
│  │  ┌─── 장 운영 시간 루프 (09:00~15:20, 매수 09:25~) ────┐    │     │
│  │  │                                        │    │     │
│  │  │  [F2] 스캔 사이클 (매 5분)              │    │     │
│  │  │       │  ※ 휴장일이면 SKIP              │    │     │
│  │  │       ├─ [F2-1] 잔고 동기화            │    │     │
...
[F1-4] 미체결 매수 주문 전량 취소
  ...
  │
  ▼
장 시작 전 준비 완료. 09:00 첫 스캔 대기. (매수는 09:25 이후)
```

---

## [F2] 스캔 사이클 (매 5분, 09:00~15:20)
...
[사전 체크 2] 현재 시간 확인
  ...
  └─ 09:00 ~ 15:20 → 정상 실행 (매수는 09:25 이후만)
```
...
[STEP 4] 손절 체크 (최우선, P0)
  ...
  │
  │  profit_rate <= STOP_LOSS_RATE (-2.0, 단타)?
  │  → YES: ★ 즉시 매도 실행 → [SELL] 으로 이동
  │
  ▼
[STEP 5] 익절 체크 (P1)
  │
  │  profit_rate >= TAKE_PROFIT_RATE (2.0, 단타)?
...
[STEP 5.5] 트레일링 스탑 체크 (P2)
  ...
  │  활성화 조건: profit_rate >= TRAILING_STOP_ACTIVATE (1.5, 단타)?
  │    → YES: 트레일링 활성화 상태
...
[STEP 5.7] 보유 기간 초과 체크 (P3)
  ...
  │  (현재 영업일 - 매수일) > MAX_HOLDING_DAYS (2, 단타)?
...
스캔 결과 요약:
  ...
  │  보유 종목: 3개
  │  매도 실행: 1건 (005930 손절 -2.1%)
  │  매수 실행: 1건 (000660 단타 13주)
  │  SKIP 사유:
  │    035720: 거래대금 부족 (8.5억 < 10억)
  │    051910: 단타 조건 미충족 (전일대비 -0.5%)
  │  에러: 0건
  │  API 호출 횟수: 17건
  │  소요 시간: 2.3초
  │
